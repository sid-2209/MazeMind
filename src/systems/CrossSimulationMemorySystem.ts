...
export class CrossSimulationMemorySystem {
  ...
  constructor() {
    console.log('🧠 CrossSimulationMemorySystem initialized');
  }
  ...
  loadMemories(agent: Agent): void {
    ...
    try {
      ...
      if (memory) {
        console.log(`✅ Loaded ${memory.performance.totalRuns} previous runs for ${agentName}`);
        ...
        console.log(`📝 No previous memories found for ${agentName} (first run)`);
  ...
  saveMemories(
    ...
    console.log(`💾 Saved memories for ${agentName} (Run #${memory.performance.totalRuns})`);
  ...
  private inheritKnowledge(agent: Agent, memory: CrossSimulationMemory): void {
    ...
    knowledge.discoveredPaths.forEach(pathInfo => {
      agent.addObservation(
        `From past runs: I know a path from (${pathInfo.from.x},${pathInfo.from.y}) to (${pathInfo.to.x},${pathInfo.to.y}). ` +
        `Used ${pathInfo.timesUsed} times with ${(pathInfo.successRate * 100).toFixed(0)}% success.`,
        Math.min(9, 5 + pathInfo.successRate * 3),
        ['cross-simulation', 'path', 'inherited'],
        pathInfo.from
      );
      inheritedCount++;
    });
    ...
    sortedDangers.slice(0, 10).forEach(danger => {
      ...
      agent.addObservation(
        `⚠️ DANGER from past runs: ${danger.type} at (${danger.location.x},${danger.location.y}). ` +
        `${danger.description} Deaths here: ${danger.deathsAtLocation}`,
        importance,
        ['cross-simulation', 'danger', 'warning', 'inherited'],
        danger.location
      );
      inheritedCount++;
    });
    ...
    knowledge.safeRooms.forEach(safeRoom => {
      agent.addObservation(
        `From past runs: Safe room at (${safeRoom.x},${safeRoom.y}). Good place to rest and recover.`,
        6,
        ['cross-simulation', 'safe-room', 'inherited'],
        safeRoom
      );
      inheritedCount++;
    });
    ...
    Object.entries(knowledge.resourceLocations).forEach(([itemType, locations]) => {
      locations.slice(0, 5).forEach(loc => { // Top 5 per type
        agent.addObservation(
          `From past runs: Found ${itemType} at (${loc.x},${loc.y}).`,
          5,
          ['cross-simulation', 'resource', itemType.toLowerCase(), 'inherited'],
          loc
        );
        inheritedCount++;
      });
    ...
    knowledge.exitLocations.forEach(exitInfo => {
      agent.addObservation(
        `🚪 EXIT LOCATION from past runs: (${exitInfo.location.x},${exitInfo.location.y})! ` +
        ...
        10,
        ['cross-simulation', 'exit', 'critical', 'inherited'],
        exitInfo.location
      );
      inheritedCount++;
    });

    console.log(`  📖 Inherited ${inheritedCount} memories from past runs`);
  }
  ...
  private generatePastReflections(agent: Agent, memory: CrossSimulationMemory): void {
    ...
    if (perf.totalRuns > 0) {
      ...
      agent.getMemoryStream().addReflection(
        `I've attempted this maze ${perf.totalRuns} times before. ` +
        `Success rate: ${successRate.toFixed(1)}%. ${advice}.`,
        8,
        ['cross-simulation', 'reflection', 'performance']
      );
      reflectionCount++;
    }
    ...
    if (bestStrategy && bestStrategy.successRate > 0.5) {
      agent.getMemoryStream().addReflection(
        `My most successful strategy is: ${bestStrategy.name}. ` +
        `${bestStrategy.description} Success: ${(bestStrategy.successRate * 100).toFixed(0)}%`,
        9,
        ['cross-simulation', 'reflection', 'strategy']
      );
      reflectionCount++;
    }
    ...
    if (trustLevels.size > 0) {
      ...
      if (trustEntries.length > 0 && trustEntries[0][1] > 0.6) {
        ...
        agent.getMemoryStream().addReflection(
          `I trust ${agentId} highly (${(trustLevel * 100).toFixed(0)}%). ` +
          `Their advice has been valuable in past runs. I should listen to them.`,
          7,
          ['cross-simulation', 'reflection', 'social']
        );
        reflectionCount++;
      }
    }
    ...
    if (perf.totalRuns >= 3) {
      ...
      agent.getMemoryStream().addReflection(
        `My recent performance is ${trend}. ` +
        ...
        7,
        ['cross-simulation', 'reflection', 'trend']
      );
      reflectionCount++;
    }

    console.log(`  🧠 Generated ${reflectionCount} reflections from past experiences`);
  }
  ...
  private updateStrategies(
    ...
    const allMemories = agent.getMemoryStream().getAllMemories();
    const planMemories = allMemories.filter(m => m.memoryType === 'plan');
    ...
    if (planMemories.length > 0) {
      ...
      if (strategy) {
        ...
        if (strategy && strategy.successRate >= 0.5 && !memory.strategies.successful.includes(strategy)) {
          memory.strategies.failed = memory.strategies.failed.filter(s => s.id !== strategy!.id);
          memory.strategies.successful.push(strategy);
        } else if (strategy && strategy.successRate < 0.5 && !memory.strategies.failed.includes(strategy)) {
  ...
  public clearMemories(agentId?: string): void {
    if (agentId) {
      ...
      console.log('🗑️  Cleared ALL agent memories');
    }
  }
}
